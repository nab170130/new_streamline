...
class MarginDetection(Strategy):

    def __init__(self, labeled_dataset, unlabeled_dataset, net, nclasses, args={}):

        super(MarginDetection, self).__init__(labeled_dataset, unlabeled_dataset, net, nclasses, args)
    ...
    def compute_average_margin_scores(self, dataset):
        ...
        average_margin_scores    = torch.zeros(len(dataset)).to(self.args["device"])
        start_idx                = 0
        for loaded_batch in dataloader:
            ...
            for cls_softmax_vectors in cls_softmaxes:
                ...
                top_2_predicted_class_confidence_per_proposal, _    = torch.topk(cls_softmax_vectors, k=2, dim=1)
                prediction_margin_per_proposal                      = top_2_predicted_class_confidence_per_proposal[:,0] - top_2_predicted_class_confidence_per_proposal[:,1]
                average_margin                                      = torch.mean(prediction_margin_per_proposal)
                average_margin_scores[start_idx]                    = average_margin
                start_idx += 1

        return average_margin_scores


    def select(self, budget):
        
        # Get the average margin of prediction between the two most likely classes for each RoI for each
        # image. Choose those images that have the smallest such margins.
        acquisition_scores  = self.compute_average_margin_scores(self.unlabeled_dataset)
