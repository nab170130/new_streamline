...
class EntropyDetection(Strategy):

    def __init__(self, labeled_dataset, unlabeled_dataset, net, nclasses, args={}):

        super(EntropyDetection, self).__init__(labeled_dataset, unlabeled_dataset, net, nclasses, args)
    ...
    def compute_uncertainty_scores(self, dataset):
        ...
        average_max_class_uncertainty_scores = torch.zeros(len(dataset)).to(self.args["device"])
        start_idx                   = 0
        for loaded_batch in dataloader:
            ...
            # Compute the average entropy score for each RoI for each image.
            for cls_softmax_vectors in cls_softmaxes:
                
                per_class_log_prob_vectors                      = torch.log2(cls_softmax_vectors)
                per_class_summands                              = per_class_log_prob_vectors * cls_softmaxes
                roi_entropies                                   = -torch.sum(per_class_summands, dim=1)
                avg_entropy                                     = torch.mean(roi_entropies)
                average_max_class_uncertainty_scores[start_idx] = avg_entropy
                start_idx += 1

        return average_max_class_uncertainty_scores


    def select(self, budget):
        
        # Get the scores of each image in the unlabeled dataset. Then, select the top budget scores.
        acquisition_scores  = self.compute_uncertainty_scores(self.unlabeled_dataset)
        score_ranking_idx   = torch.argsort(acquisition_scores, descending=True)
